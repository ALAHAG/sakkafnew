...
class ControllerPaymentMobi extends Controller {
    ...
    $data['link_send_payment'] = $this->url->link('payment/mobi/send');
		...
		$this->document->setTitle('الدفع عن طريق البطاقات الائتمانية');
		$this->document->setDescription(" الدفع اونلاين عبر البطاقات الائتمانية او البيبال ");
...
if(isset($this->request->post['submit']))
{
  ...
  $encrypted = base64_encode(openssl_encrypt($pass, $aes, $key, OPENSSL_RAW_DATA, $iv));
    ...
    else
    {
		...
		$firstName = $this->request->post['nameOnCard'];
		$lastName = $this->request->post['nameOnCard'];
		$postalCode=333;
		$shippingState="KL";
// "firstName"=>$this->request->post['firstName'], "lastName"=>$this->request->post['lastName'],  "postalCode"=>$this->request->post['postalCode'],"shippingState"=>$this->request->post['shippingState'],
	$data=array("service"=>"ECOMAPI_SALE_REQ","ip"=>$this->request->post['ip'],"firstName"=>$firstName,"lastName"=>$lastName,"postalCode"=>$postalCode,"shippingState"=>$shippingState,"mUrl"=>$this->request->post['mUrl'],"nameOnCard"=>$this->request->post['nameOnCard'],"orderId"=>$this->request->post['ordrid'], "amount"=>$famt, "email"=>$this->request->post['emailid'], "carddetails"=>strtoupper(implode(unpack("H*", $encrypted))));
