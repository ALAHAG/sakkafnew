...
class ControllerContentHotels extends Controller {
	...
	public function index() {
		$this->document->setTitle('فنادق ماليزيا - احجز فنادق في كولالمبور و جزيرة لنكاوي و بينانج و كاميرون هايلاند ماليزيا وجنتنق هايلاند ماليزيا');
		$this->document->setDescription("أسعار فنادق ماليزيا - احجز فنادق في كولالمبور و جزيرة لنكاوي و بينانج و كاميرون هايلاند ماليزيا وجنتنق هايلاند ماليزيا");//$this->config->get('config_meta_description')
		$this->document->setKeywords("اسعار فنادق ماليزيا , فنادق كولالمبور , فنادق لنكاوي , اسعار فنادق بينانج , فنادق كاميرون هايلان , عروض فنادق ماليزيا , فنادق 5 نجوم في ماليزيا");//$this->config->get('config_meta_keyword')
		...
		foreach ($hotels as $key => $value) {
			$hotels[$key]['thumb'] = $this->model_tool_image->resize($value['image'], 270, 200,'w');
				$hotels[$key]['city_name'] = $this->model_localisation_zone->getCity($value['city_id'])['city_ar_name'];
	...
	public function show() {
		...
		if (isset($this->request->get['content_id'])) {
			...
			if ($hotel_info) {
				...
				$data['hotel_address'] =$hotel_info['address'];
				...
				foreach ($hotel_images as $key => $value) {
					...
					if($hotel_images[$key]['cover']=='1'){
						$hotel_image_cover = $value['image'];
					}
				}

				$data['hotel_images'] =$hotel_images;
				if(empty($hotel_image_cover)){
					$hotel_image_cover =$hotel_images[0]['image'];
				}
				$data['hotel_image_cover']	= $hotel_image_cover;
				// Hotel Rooms
