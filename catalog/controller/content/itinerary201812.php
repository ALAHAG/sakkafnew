...
class ControllerContentItinerary extends Controller {
	...
	public function index() {
		...
		$this->document->addScript('//code.jquery.com/ui/1.11.4/jquery-ui.js');
		...
		if (isset($this->request->get['page'])) {
					...
					header("location:https://alsakkaftravel.net/packages");
					die();
		...
		if (isset($this->request->get['category'])) {
					$array_category = $this->request->get['category'];
					if ( filter_var($array_category, FILTER_VALIDATE_INT) === false ) {

					setcookie("error", "Invalid page.", time()+3600);
					header("location:https://alsakkaftravel.net/packages");
					die();
					// var_dump("$array_category is not int");

				}
			$filters['category'] = $this->request->get['category'];
		}
		if (isset($this->request->get['night'])) {
					$array_night = $this->request->get['night'];
					if ( filter_var($array_night, FILTER_VALIDATE_INT) === false ) {

					setcookie("error", "Invalid page.", time()+3600);
					header("location:https://alsakkaftravel.net/packages");
					die();
					// var_dump("$array_night is not int");
				}
