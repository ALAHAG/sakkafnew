...
class ControllerContentTestimonial extends Controller { 
	...
	public function index() {
		...
		$this->document->setTitle($this->language->get('قالوا عن شركة السقاف للسياحة والسفر ماليزيا'));
		...
		$this->document->setKeywords("أراء عملاء السقاف للسياحة والسفر , قالوا عننا , السقاف للسياحة , عملاء السقاف , وكلاء السقاف");//$this->config->get('config_meta_keyword')
	...
	protected function getList() {
		...
		// }	
			...
			$results[$key]['thumb'] = $this->model_tool_image->resize($value['image'], 300, 300,'h');
		}
		...
		$pagination->limit = 12;
		$link =  "//$_SERVER[HTTPS_HOST]$_SERVER[REQUEST_URI]";
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($information_total) ? (($page - 1) * 12) + 1 : 0, ((($page - 1) * 12) > ($information_total - 12)) ? $information_total : ((($page - 1) * 12) + 12), $information_total, ceil($information_total / 12));
