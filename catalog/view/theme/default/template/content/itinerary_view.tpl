   ...
   <body  class="bg-color" style="background:#7f1f5f">
     <div class="container">
      ...
      <div style="clear:both;">
         ...
         <br>
         <form class="" action="https://alsakkaftravel.net/tpayment" method="post">
         ...
         <table class="program" style="margin: 0 auto; margin-top: 2em;border:1px solid #fff; min-width:80%;" align="center" dir="rtl" cellpadding="0" cellspacing="0">
            ...
            <tbody>
               ...
               <tr>
                  <td class="tb_col" style="text-align: right;padding:10px;font-weight:bold;    font-size: 1.2em !important;color:#7B0462;"><label>
                     رسوم التحويل
                     <sub> <small>ملاحظة:رسوم التحويل عبر بطاقة الائتمان 2.4% من المبلغ المراد تحويله</small></sub>
                  </td>
                  <td class="tb_col" style="text-align: center;padding:10px;font-weight:bold;    font-size: 1.2em !important;color:#7B0462;"><label>
                     <input type="button" disabled class="disable" value="<?php echo ceil($customer_price*0.024) ?>" name="trab" id="trab">
                    <?php echo $currancy[$currancy_type];?>
                  </td>
               </tr>
               <tr style="background:#f4e6f9">
                  ...
                  <td class="tb_col" style="text-align: center;padding:10px;font-weight:bold; font-size: 1.2em !important;color:#7B0462;"><label>
                     <input type="button" class="disable"disabled value="<?php echo ceil($customer_price+$customer_price*0.024) ?>" name="xtrab" id="xtrab">
                    <?php echo $currancy[$currancy_type];?>
                  </td>
               </tr>
               <tr>
                  <td colspan="2">
                    <img src="image/paymentgateway.png" title="الدفع اونلاين عبر السقاف للسياحة والسفر" alt="الدفع اونلاين عبر السقاف للسياحة والسفر">
                    <br/><input type="submit" name="submit" value="الدفع عبر البطاقة الائتمانية" id="button-confirm" class="full-width btn-large">
                  </td>
               </tr>
            </tbody>
         </table>
       ...
       <?php endif?>
        <?php if(empty($currancy[$currancy_type]) || $currancy[$currancy_type] =="رنجت ماليزي" ){?>
         <input type="hidden" value="<?php echo ceil($customer_price) ?>" name="prices" id="prices">
         <input type="hidden" value="<?php echo ceil($customer_price*0.024) ?>" name="taxes" id="taxes">
         <input type="hidden" value="<?php echo ceil($customer_price+$customer_price*0.024) ?>" name="total_payment" id="total_payment">
         <input type="hidden" value="<?php echo $currancy[$currancy_type];?>" name="currancy" id="currancy">
         <!-- <div id="paypal-button-container" style="margin:0 auto ;"></div> -->
       <?php }else{?>

         <input type="hidden" value="0.00" name="prices" id="pricess">
         <input type="hidden" value="0.00" name="taxes" id="taxess">
         <input type="hidden" value="0.00" name="total_payment" id="total_payments">
         <input type="hidden" value="0.00" name="currancy" id="currancys">
         <input type="hidden" value="0.00" name="payment-option" id="currancys">

        <?php }?>
         <input type="hidden" value="<?php echo 'B'.$booking_id ?>" name="package_ids" id="package_ids">
         <input type="hidden" value="<?php echo (empty($customer_name_en))? '' : $customer_name_en; ?>" name="customer_name" id="name">
       </form>

         <div class="payments_raw">
           <?php echo $payment_mobi; ?>
         </div>
                  ...
                  document.querySelectorAll('input[name=payment-option]')
                        .forEach(function (el) {
                          el.addEventListener('change', function (event) {
                            ...
                            document.getElementById("taxes").value = Math.ceil(event.target.value*0.024);
                            document.getElementById("total_payment").value = Math.ceil(event.target.value*1 + event.target.value*0.024);
                            document.getElementById("xtrab").value = Math.ceil(event.target.value*1 + event.target.value*0.024);
                            document.getElementById("nprice").value = Math.ceil(event.target.value*1);
                            document.getElementById("trab").value = Math.ceil(event.target.value*0.024);
                          });
                        });
