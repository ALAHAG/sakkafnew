...
class ControllerContentTestimonial extends Controller { 
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('content/testimonial', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			// print_r($_POST);exit();
			$this->model_content_testimonial->editInformation($this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $information_id) {
				$this->model_content_testimonial->deleteInformation($information_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('content/testimonial', 'token=' . $this->session->data['token'] . $url, 'SSL')
			);
		
		$data['insert'] = $this->url->link('content/testimonial/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('content/testimonial/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['repair'] = $this->url->link('content/testimonial/repair', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$information_total = $this->model_content_testimonial->getTotalInformations();

		$data['informations']=array();
		$results = $this->model_content_testimonial->getInformations($data);
		foreach ($results as $result) {
			
			$data['informations'][] = array(
				'information_id' => $result['testimonial_id'],
				...
				'edit'        => $this->url->link('content/testimonial/edit', 'token=' . $this->session->data['token'] . '&testimonial_id=' . $result['testimonial_id'] . $url, 'SSL'),
				'delete'      => $this->url->link('content/testimonial/delete', 'token=' . $this->session->data['token'] . '&testimonial_id=' . $result['testimonial_id'] . $url, 'SSL')
			);
		}	
		...
		$data['sort_name'] = $this->url->link('content/testimonial', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('content/testimonial', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
		...
		$data['information_total']=$information_total;
		// dd($information_total);
		$pagination = new Pagination();
		$pagination->total = $information_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('content/testimonial', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['testimonial_id'])) {
			$data['action'] = $this->url->link('content/testimonial/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('content/testimonial/edit', 'token=' . $this->session->data['token'] . '&testimonial_id=' . $this->request->get['testimonial_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('content/testimonial', 'token=' . $this->session->data['token'] . $url, 'SSL');
		if (isset($this->request->get['testimonial_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$information_info = $this->model_content_testimonial->getInformation($this->request->get['testimonial_id']);
		}
		...
		if (isset($this->request->post['country_id'])) {
			...
			$data['country_id'] = $information_info['country_id'];
		} else {
			$data['country_id'] = '';
		}
		...
		if (isset($this->request->post['customer_name'])) {
			$data['customer_name'] = $this->request->post['name'];
		} elseif (!empty($information_info)) {
			$data['customer_name'] = $information_info['name'];
		} else {
			$data['customer_name'] = '';
		}
		...
		if (isset($this->request->post['customer_id'])) {
			...
			$data['customer_id'] = $information_info['customer_id'];
		} else {
			$data['customer_id'] = '';
		}
		...
		if (isset($this->request->post['email'])) {
			...
			$data['email'] = $information_info['email'];
		} else {
			$data['email'] = '';
		}
		...
		// } elseif (!empty($information_info)) {
		// 	$data['language_id'] = $information_info['language_id'];
		// } else {
		// 	$data['language_id'] = '';
		// }
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'content/testimonial')) {
