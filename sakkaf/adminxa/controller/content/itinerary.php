...
class ControllerContentItinerary extends Controller {
	...
	public function getForm($data = array())
	{
		...
		$data['get_tours'] = $this->url->link('content/tours/getTours&token=' . $this->session->data['token'], '' , 'SSL');
		...
		//tours order
		if (isset($this->request->get['booking_type'])) {
			$itinerary_tours = $this->model_content_itinerary->getTours($this->request->get['booking_id']);
			$data['itinerary_class_id'] = '2'; //$this->itinerary_class_id;//
		...
		if ($itinerary) {
			...
			// dd($data['itinerary']);
		}else if (isset($this->request->get['booking_type'])) {
			foreach ($itinerary_tours as $key => $tour) {
					$itinerary_tours[$key]['tour_image']= $itinerary_tours[$key]['image'];
					$itinerary_tours[$key]['tour_title_ar']=$itinerary_tours[$key]['title_ar'];
					$itinerary_tours[$key]['tour_title_en']=$itinerary_tours[$key]['title_en'];
					$itinerary_tours[$key]['tour_description_ar']= $itinerary_tours[$key]['description_ar'];
					$itinerary_tours[$key]['tour_description_en']=$itinerary_tours[$key]['description_en'];
					$itinerary_tours[$key]['price']=$itinerary_tours[$key]['selprice'];
					$itinerary_tours[$key]['type'] = 'tour';
					$itinerary_tours[$key]['order'] = $tour['order'];
					if($arrival_date){
						$date = date('Y-m-d', strtotime($arrival_date. ' + '.( $tour['start_day'] - 1 ).' days'));
	...
	// dd($data);
		...
		$data['tours'] =  $this->model_content_tours->getTourCities(array('cat_id' =>'1'));
		$data['transportations'] =  $this->model_content_tours->getTourCities(array('cat_id' =>'2'));
		// dd($data['transportations']);
		if ($itinerary) {
		...
		if ($itinerary) {
			$data['entry_num_passengers'] = $itinerary['entry_num_passengers'];
		}else if(isset($this->request->post['entry_num_passengers'])){
			$data['entry_num_passengers'] = $this->request->post['entry_num_passengers'];
		}else{
			$data['entry_num_passengers'] = '';
		}
		if ($itinerary) {
		...
		if ($itinerary) {
			$data['downpayment'] = $itinerary['downpayment'];
		}else if(isset($this->request->post['downpayment'])){
			$data['downpayment'] = $this->request->post['downpayment'];
		}else{
			$data['downpayment'] = 0;
		}
		if ($itinerary) {
		...
		}else if(isset($data['viewItrinaryWp']) && $data['viewItrinaryWp'] == 'view'){
					$this->response->setOutput($this->load->view('content/itinerary_view.tpl', $data));

			}else if(isset($data['viewItrinaryWp']) && $data['viewItrinaryWp'] == 'pdf'){
					return $this->load->view('content/itinerary_view_wt.tpl', $data);
	...
	public function sendItrinaryWp()
	{
		$data['viewItrinaryWp'] = 'pdf';
...
// <center><img width="100%" src="http://alsakkaftravel.net/image//itinerary_header.jpg"></center>
//
...
//       <img src="http://alsakkaftravel.net/image/catalog/packages/smll car.jpg" width="100%">
...
//
//       <div style="  width: 150px;height: 180px;float: right;margin: 5px;border: 1px solid rgba(0,0,0,0.3);">
//       <img src="http://alsakkaftravel.net/image/catalog/packages/air.jpg" width="100%">
