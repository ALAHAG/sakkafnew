...
class ControllerContentBooking extends Controller {
	...
	public function getForm()
	{
		...
		if (isset($this->request->get['booking_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			if($booking_info[booking_type]=='88'){
				// dd(	$booking_info);
				$data['create_quotation'] = $this->url->link('content/itinerary/editquotation&itinerary_id='.$booking_info['itinerary_id'].'&booking_type='.$booking_info['booking_type'].'&booking_id='.$booking_info['booking_id'], 'token=' . $this->session->data['token'] , 'SSL');
			...
			$data['link_send_itineraryWp'] = $this->url->link('content/itinerary/sendItrinaryWp&booking_id='.$booking_info['booking_id'].'&token=' . $this->session->data['token'] , '' , 'SSL');
