	...
	*/
	...
	if( !function_exists('kode_get_breadcumbs') ){	
		function kode_get_breadcumbs () {
			...
			$id         = 'breadcrumbs';
			$class      = 'breadcrumbs';
			$home_title = __('Homepage','kodeforest');
			 $parents = '';
			...
			if ( !is_front_page() ) {
				...
				if ( is_single() ) {
					...
					// Search results page
					echo '<li class="item-current item-current-' . esc_attr(get_search_query()) . '"><strong class="bread-current bread-current-' . esc_attr(get_search_query()) . '" title="Search results for: ' . esc_attr(get_search_query()) . '">Search results for: ' . esc_attr(get_search_query()) . '</strong></li>';
	...
	if(!function_exists('kode_numbers_list')){
		function kode_numbers_list($numbers_list_for){
			...
			if($searched_value == 'any' || empty($searched_value)) {
				$html .= '<option value="any" selected="selected">'.__( 'Any', 'KodeForest').'</option>';
			} else {
				$html .= '<option value="any">'.__( 'Any', 'KodeForest').'</option>';
			}
			return $html;
		}
	}
	...
	function kode_search_form_banner($title=""){
		...
		<div class="kd-tourform">
			...
			<form class="advance-search-form" action="'.esc_url(get_permalink($theme_option['package-search-page'])).'" method="get">
				<ul>
					<li>	
						<span class="keyword-txt">'. __('Keyword', 'kodeforest').'</span>
						<input type="text" name="keyword" id="keyword-txt" value="'.$package_keyword.'" placeholder="'. __('Any', 'kodeforest').'" />
					</li>
					<li>
						<span class="select-days">'. __('Days', 'kodeforest').'</span>
					...
					<li>
						<span class="select-min-price">'. __('Min Price', 'kodeforest').'</span>
					...
					<li>
						<span class="select-min-price">'. __('Max Price', 'kodeforest').'</span>
						<label for="select-min-price">
					...
					<li class="k-submit-btn"><input type="submit" class="thbg-color" value="'.__('Search Now','kodeforest').'"></li>
				</ul>
			</form>
		</div>
		</div>
