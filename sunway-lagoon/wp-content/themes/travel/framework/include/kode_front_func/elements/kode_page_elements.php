		...
		function kode_get_default_content_item( $settings ){
			...
			while ( have_posts() ){ the_post();
				$content = kode_content_filter(get_the_content(), true); 
					...
					<?php 
					global $post;
					//Author box condition on page start
					$kode_post_option = json_decode(kode_decode_stopbackslashes(get_post_meta($post->ID, 'post-option', true)),true);
					if(isset($kode_post_option['page-author-box']) AND $kode_post_option['page-author-box'] == 'disable'){}else{ ?>
					...
					<?php }
					//Author box condition on page Ends
					?>
					<?php
					if ( comments_open() || get_comments_number() ) :
						comments_template();
